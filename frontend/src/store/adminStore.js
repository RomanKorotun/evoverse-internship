...
import { updateUserStatus, updateUserQuota, getUsers } from "../api/adminApi";
...
export const adminStore = create((set, get) => ({
  ...
  // змінити квоту (локально оновлюємо тільки quota)
  updateQuota: async (userId, quota) => {
    try {
      await updateUserQuota(userId, quota);

      set((state) => ({
        users: state.users.map((u) => (u.id === userId ? { ...u, quota } : u)),
      }));
    } catch (err) {
